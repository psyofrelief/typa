...
const Test = () => {
  const { userName, isLoggedIn, isMobile, setIsMobile } = useMyContext();
  ...
  return (
    <div id="page--test">
      ...
      <div className="top-row">
        {!isTyping && (
          <button className="btn-start-test" onClick={startTest} tabIndex={0}>
            Start Test
          </button>
        )}

        {activeTimerOption && <p className="timer">{timeLeft}</p>}
        <button className="btn-reset-test" onClick={() => resetTest()}>
          Reset
        </button>
      </div>
